...
static const auto GENERATOR_DOES_NOT_SUPPORT_TENSOR_DEVICE_TYPE =
    "generator does not support tensor device type";
static const auto TENSOR_DEVICE_TYPE_IS_NOT_SUPPORTED =
...
Tensor& random_to(Tensor& self, int64_t to, std::optional<Generator> gen) {
  if (self.device().type() == DeviceType::CPU) {
...
Tensor&
uniform_(Tensor& self, double from, double to, std::optional<Generator> gen) {
  if (self.device().type() == DeviceType::CPU) {
...
Tensor&
normal_(Tensor& self, double mean, double std, std::optional<Generator> gen) {
  if (self.device().type() == DeviceType::CPU) {
...
Tensor& normal_Tensor_Tensor_out(
  ...
  if (output.device().type() == DeviceType::CPU) {
    ...
    return torch::csprng::cuda::normal_Tensor_Tensor_out(
        output, mean, std, gen);
#endif
  } else {
    TORCH_CHECK(false, GENERATOR_DOES_NOT_SUPPORT_TENSOR_DEVICE_TYPE);
...
Tensor&
exponential_(Tensor& self, double lambda, std::optional<Generator> gen) {
  if (self.device().type() == DeviceType::CPU) {
...
// randperm implementation was copied from PyTorch to unblock CSPRNG users, but
// ultimately CSPRNG must reuse refactored randperm from PyTorch, see
// https://github.com/pytorch/pytorch/issues/43816
