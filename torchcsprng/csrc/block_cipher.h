...
TORCH_CSPRNG_HOST_DEVICE static void copy_input_to_block_non_cont(int64_t idx, uint8_t* block, int block_size,
...
template<typename input_index_calc_t>
TORCH_CSPRNG_HOST_DEVICE static void copy_input_to_block_cont(int64_t idx, uint8_t* block, int block_size,
    void* input_ptr, int64_t input_numel, int input_type_size, input_index_calc_t input_index_calc) {
  if (idx * (block_size / input_type_size) + block_size / input_type_size - 1 < input_numel) {
    const auto linear_index = idx * (block_size / input_type_size);
    std::memcpy(
        block,
        &(reinterpret_cast<uint8_t*>(input_ptr)[input_index_calc(linear_index)]),
        block_size
...
TORCH_CSPRNG_HOST_DEVICE static void copy_block_to_output_non_cont(int64_t idx, uint8_t* block, int output_elem_per_block,
...
template<typename output_index_calc_t>
TORCH_CSPRNG_HOST_DEVICE static void copy_block_to_output_cont(int64_t idx, uint8_t* block, int output_elem_per_block,
    void* output_ptr, int64_t output_numel, int output_type_size, output_index_calc_t output_index_calc) {
  if (idx * output_elem_per_block + output_elem_per_block - 1 < output_numel) {
    const auto linear_index = idx * output_elem_per_block;
    std::memcpy(
        &(reinterpret_cast<uint8_t*>(output_ptr)[output_index_calc(linear_index)]),
        block,
        output_type_size * output_elem_per_block
    );
  }
}

template<int block_size, typename cipher_t, typename input_index_calc_t, typename output_index_calc_t, typename transform_t>
TORCH_CSPRNG_HOST_DEVICE static void block_cipher_kernel_helper_cont_cont(
  ...
  if (input_ptr != nullptr) {
    copy_input_to_block_cont(idx, block, block_size, input_ptr, input_numel, input_type_size, input_index_calc);
  }
  cipher(idx, block);
  transform(block);
  copy_block_to_output_cont(idx, block, output_elem_per_block, output_ptr, output_numel, output_type_size, output_index_calc);
}

template<int block_size, typename cipher_t, typename input_index_calc_t, typename output_index_calc_t, typename transform_t>
TORCH_CSPRNG_HOST_DEVICE static void block_cipher_kernel_helper_non_cont_cont(
    int64_t idx, cipher_t cipher, int output_elem_per_block,
    void* input_ptr, int64_t input_numel, int input_type_size, input_index_calc_t input_index_calc,
    void* output_ptr, int64_t output_numel, int output_type_size, output_index_calc_t output_index_calc,
    transform_t transform) {
  ...
  std::memset(&block, 0, block_size); // is it ok to use zeros as padding?
  if (input_ptr != nullptr) {
    copy_input_to_block_non_cont(idx, block, block_size, input_ptr, input_numel, input_type_size, input_index_calc);
  }
  cipher(idx, block);
  transform(block);
  copy_block_to_output_cont(idx, block, output_elem_per_block, output_ptr, output_numel, output_type_size, output_index_calc);
...
template<int block_size, typename cipher_t, typename input_index_calc_t, typename output_index_calc_t, typename transform_t>
TORCH_CSPRNG_HOST_DEVICE static void block_cipher_kernel_helper_cont_non_cont(
    int64_t idx, cipher_t cipher, int output_elem_per_block,
    void* input_ptr, int64_t input_numel, int input_type_size, input_index_calc_t input_index_calc,
    void* output_ptr, int64_t output_numel, int output_type_size, output_index_calc_t output_index_calc,
    transform_t transform) {
  ...
  cipher(idx, block);
  transform(block);
  copy_block_to_output_non_cont(idx, block, output_elem_per_block, output_ptr, output_numel, output_type_size, output_index_calc);
}

template<int block_size, typename cipher_t, typename input_index_calc_t, typename output_index_calc_t, typename transform_t>
TORCH_CSPRNG_HOST_DEVICE static void block_cipher_kernel_helper_non_cont_non_cont(
    int64_t idx, cipher_t cipher, int output_elem_per_block,
    void* input_ptr, int64_t input_numel, int input_type_size, input_index_calc_t input_index_calc,
    void* output_ptr, int64_t output_numel, int output_type_size, output_index_calc_t output_index_calc,
    transform_t transform) {
...
__global__ static void block_cipher_kernel_cuda(cipher_t cipher, int output_elem_per_block,
  ...
  block_cipher_kernel_helper_cont_cont<block_size>(idx, cipher, output_elem_per_block,
    input_ptr, input_numel, input_type_size, input_index_calc,
    output_ptr, output_numel, output_type_size, output_index_calc,
    transform, copy_input_to_block_non_cont, copy_block_to_output_non_cont);
}
#endif
...
static void block_cipher_kernel_cpu_serial(int64_t begin, int64_t end, cipher_t cipher, int output_elem_per_block,
    ...
    void* output_ptr, int64_t output_numel, int output_type_size, output_index_calc_t output_index_calc, bool output_is_contiguous,
    transform_t transform) {
  if (input_is_contiguous && output_is_contiguous) {
  ...
  } else if (input_is_contiguous) {
    for (auto idx = begin; idx < end - 1; ++idx) {
      block_cipher_kernel_helper_cont_non_cont<block_size>(idx, cipher, output_elem_per_block,
        input_ptr, input_numel, input_type_size, input_index_calc,
        output_ptr, output_numel, output_type_size, output_index_calc,
        transform);
    }
  } else if (output_is_contiguous) {
    for (auto idx = begin; idx < end - 1; ++idx) {
      block_cipher_kernel_helper_non_cont_cont<block_size>(idx, cipher, output_elem_per_block,
        input_ptr, input_numel, input_type_size, input_index_calc,
        output_ptr, output_numel, output_type_size, output_index_calc,
        transform);
    }
    ...
    for (auto idx = begin; idx < end - 1; ++idx) {
      block_cipher_kernel_helper_non_cont_non_cont<block_size>(idx, cipher, output_elem_per_block,
        input_ptr, input_numel, input_type_size, input_index_calc,
        output_ptr, output_numel, output_type_size, output_index_calc,
        transform);
    }
  }
  block_cipher_kernel_helper_non_cont_non_cont<block_size>(end - 1, cipher, output_elem_per_block,
    input_ptr, input_numel, input_type_size, input_index_calc,
    output_ptr, output_numel, output_type_size, output_index_calc,
    transform);
}
