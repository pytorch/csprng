...
TORCH_CSPRNG_HOST_DEVICE static void copy_input_to_block(
    ...
    int64_t input_numel,
    int input_type_size,
    input_index_calc_t input_index_calc) {
    ...
    if (linear_index < input_numel) {
      std::memcpy(
          ...
          &(reinterpret_cast<uint8_t*>(
              input_ptr)[input_index_calc(linear_index)]),
          input_type_size);
    }
  }
}
...
TORCH_CSPRNG_HOST_DEVICE static void copy_block_to_output(
    int64_t idx,
    uint8_t* block,
    int output_elem_per_block,
    void* output_ptr,
    int64_t output_numel,
    int output_type_size,
    output_index_calc_t output_index_calc) {
  for (auto i = 0; i < output_elem_per_block; ++i) {
    const auto linear_index = idx * output_elem_per_block + i;
    if (linear_index < output_numel) {
      std::memcpy(
          &(reinterpret_cast<uint8_t*>(
              output_ptr)[output_index_calc(linear_index)]),
...
template <
    int block_size,
    typename cipher_t,
    typename input_index_calc_t,
    typename output_index_calc_t,
...
template <
    ...
    typename output_index_calc_t,
    typename transform_t>
__global__ static void block_cipher_kernel_cuda(
    cipher_t cipher,
    int output_elem_per_block,
    void* input_ptr,
    int64_t input_numel,
  ...
  block_cipher_kernel_helper<block_size>(
      idx,
      cipher,
      output_elem_per_block,
      input_ptr,
...
template <
    ...
    typename output_index_calc_t,
    typename transform_t>
static void block_cipher_kernel_cpu_serial(
    int64_t begin,
    int64_t end,
    cipher_t cipher,
    int output_elem_per_block,
...
template <
    ...
    typename output_index_calc_t,
    typename transform_t>
static void block_cipher_kernel_cpu(
    int64_t total,
    cipher_t cipher,
    int output_elem_per_block,
    void* input_ptr,
  ...
  if (total < at::internal::GRAIN_SIZE || at::get_num_threads() == 1) {
    block_cipher_kernel_cpu_serial<block_size>(
        0,
        total,
        cipher,
        output_elem_per_block,
    ...
    at::parallel_for(
        0, total, at::internal::GRAIN_SIZE, [&](int64_t begin, int64_t end) {
          block_cipher_kernel_cpu_serial<block_size>(
              begin,
              end,
              cipher,
...
void block_cipher(
  ...
  if (device.type() == at::kCPU) {
    const auto total =
        (output_numel + output_elem_per_block - 1) / output_elem_per_block;
    block_cipher_kernel_cpu<block_size>(
        total,
        cipher,
        output_elem_per_block,
        input_ptr,
    ...
    const auto grid = (output_numel + (threads * output_elem_per_block) - 1) /
        (threads * output_elem_per_block);
...
void block_cipher(at::Tensor input, at::Tensor output, cipher_t cipher) {
  ...
  const auto input_offset_calc =
      make_offset_calculator<1>(at::TensorIterator::nullary_op(input));
  const auto input_index_calc =
      [input_offset_calc] TORCH_CSPRNG_HOST_DEVICE(uint32_t li) -> uint32_t {
  ...
  const auto output_offset_calc =
      make_offset_calculator<1>(at::TensorIterator::nullary_op(output));
  const auto output_index_calc =
      [output_offset_calc] TORCH_CSPRNG_HOST_DEVICE(uint32_t li) -> uint32_t {
