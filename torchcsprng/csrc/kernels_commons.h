...
// CUDA CSPRNG is actually CPU generator which is used only to generate a random
// key on CPU for AES running in a block mode on CUDA
struct CSPRNGGeneratorImpl : public c10::GeneratorImpl {
  CSPRNGGeneratorImpl(bool use_rd)
      : c10::
            GeneratorImpl{at::Device(at::DeviceType::CPU), at::DispatchKeySet(at::DispatchKey::CustomRNGKeyId)},
        use_rd_{use_rd} {}
  ...
  uint64_t random64() {
    return use_rd_ ? make64BitsFrom32Bits(rd_(), rd_())
                   : make64BitsFrom32Bits(mt_(), mt_());
  }
